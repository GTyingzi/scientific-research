function gui_extract_matrix()
% This is the machine-generated representation of a MATLAB object
% and its children.  Note that handle values may change when these
% objects are re-created. This may cause problems with some callbacks.
% The command syntax may be supported in the future, but is currently 
% incomplete and subject to change.
%
% To re-open this system, just type the name of the m-file at the MATLAB
% prompt. The M-file and its associtated MAT-file must be on your path.


a = figure('Units','pixels', ...
	'MenuBar','none', ...
	'Position',[20 80 300 250], ...
	'HandleVisibility','callback', ...
	'Tag','Fig_gui_extract_matrix');
    
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.342857142857143, ...
	'ListboxTop',0, ...
	'Position',[0.02 0.84 0.96 0.14], ...
	'Style','frame', ...
	'Tag','Frame1');    
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.872727272727273, ...
	'FontWeight','bold', ...
	'HorizontalAlignment','center', ...
	'Position',[0.1 0.87 0.8 0.055], ...
	'String','Matrix Extraction', ...
	'Style','text', ...
	'Tag','StaticText1');
    
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.0695652173913043, ...
	'ListboxTop',0, ...
	'Position',[0.02 0.14 0.96 0.69], ...
	'Style','frame', ...
	'Tag','Frame2');    
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.872727272727273, ...
	'FontWeight','bold', ...
	'HorizontalAlignment','center', ...
	'Position',[0.1 0.76 0.8 0.055], ...
	'String','Matrix Data', ...
	'Style','text', ...
	'Tag','StaticText1');

    % Input
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.685714285714286, ...
	'HorizontalAlignment','left', ...
	'Position',[0.05 0.65 0.18 0.07], ...
	'String','Input', ...
	'Style','text', ...
	'Tag','StaticText1');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'BackgroundColor',[1 1 1], ...
	'FontSize',0.685714285714286, ...
	'Position',[0.27 0.66 0.49 0.07], ...
	'Style','edit', ...
	'Tag','Text_input');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'Callback','fl_create_structures(''get_input_name'');', ...
	'FontSize',0.6, ...
	'Position',[0.79 0.66 0.16 0.07], ...
	'String','Refresh', ...
	'Tag','Pushbutton1');

    
    % output
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.685714285714286, ...
	'HorizontalAlignment','left', ...
	'Position',[0.05 0.53 0.18 0.07], ...
	'String','Output', ...
	'Style','text', ...
	'Tag','StaticText1');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'BackgroundColor',[1 1 1], ...
	'FontSize',0.685714285714286, ...
	'Position',[0.27 0.54 0.49 0.07], ...
	'Style','edit', ...
	'Tag','Text_output');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'Callback','fl_create_structures(''get_output_name'');', ...
	'FontSize',0.6, ...
	'Position',[0.79 0.54 0.16 0.07], ...
	'String','Refresh', ...
	'Tag','Pushbutton1');

b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'BackgroundColor',[1 1 1], ...
	'Callback','fl_create_structures(''edit_firstLine'');', ...
	'FontSize',0.6, ...
	'ListboxTop',0, ...
	'String','1', ...
	'Position',[0.36    0.33    0.16    0.07], ...
	'Style','edit', ...
	'Tag','EditText_firstLine');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'BackgroundColor',[1 1 1], ...
	'Callback','fl_create_structures(''edit_lastLine'');', ...
	'FontSize',0.6, ...
	'ListboxTop',0, ...
	'Position',[0.63    0.33    0.16    0.07], ...
	'Style','edit', ...
	'Tag','EditText_lastLine');
    
b = uicontrol('Parent',a, ...
        'Units','normalized', ...
	'FontUnits','normalized', ...
	'BackgroundColor',[1 1 1], ...
	'Callback','fl_create_structures(''edit_firstRow'');', ...
	'FontSize',0.6, ...
	'ListboxTop',0, ...
	'String','1', ...
	'Position',[ 0.36 0.22 0.16    0.07], ...
	'Style','edit', ...
	'Tag','EditText_firstRow');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'BackgroundColor',[1 1 1], ...
	'Callback','fl_create_structures(''edit_range_lastRow'');', ...
	'FontSize',0.6, ...
	'ListboxTop',0, ...
	'Position',[0.63    0.22    0.16    0.07], ...
	'Style','edit', ...
	'Tag','EditText_lastRow');
    
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.96, ...
	'ListboxTop',0, ...
	'Position',[0.3431    0.42    0.2    0.05], ...
	'String','first', ...
	'Style','text', ...
	'Tag','StaticText_first');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.96, ...
	'ListboxTop',0, ...
	'Position',[0.6161    0.42    0.2    0.05], ...
	'String','last', ...
	'Style','text', ...
	'Tag','StaticText_last');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.685714285714286, ...
	'ListboxTop',0, ...
	'Position',[0.07    0.32    0.25    0.07], ...
	'String','lines', ...
	'Style','text', ...
	'Tag','StaticText_lines');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'FontSize',0.685714285714286, ...
	'ListboxTop',0, ...
	'Position',[0.07    0.21    0.25    0.07], ...
	'String','rows', ...
	'Style','text', ...
	'Tag','StaticText_rows');

   
    % general
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'Callback','eval([''global '' fl_create_structures(''create_SUB_matrix'');]);', ...
	'FontSize',0.436363636363636, ...
	'FontWeight','bold', ...
	'Position',[0.1 0.02 0.3 0.11], ...
	'String','Create', ...
	'Tag','Pushbutton_Create');
b = uicontrol('Parent',a, ...
	'Units','normalized', ...
	'FontUnits','normalized', ...
	'Callback','fl_function(''close'');', ...
	'FontSize',0.436363636363636, ...
	'FontWeight','bold', ...
	'Position',[0.6 0.02 0.3 0.11], ...
	'String','Close', ...
	'Tag','Pushbutton_close');
	
fl_window_init(a);
