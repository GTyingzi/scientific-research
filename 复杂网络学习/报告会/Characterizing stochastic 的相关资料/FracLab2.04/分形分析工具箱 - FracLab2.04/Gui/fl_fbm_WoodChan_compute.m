function varargout=fl_fbm_WoodChan_compute(varargin)

% Callback functions for FBM GUI - Generation Parameter Window.


Figfbm = gcf;
if ((isempty(Figfbm)) | (~strcmp(get(Figfbm,'Tag'),'Fig_gui_fl_fbm_WoodChan')))
  Figfbm = findobj ('Tag','Fig_gui_fl_fbm_WoodChan');
end
  ...
  case 'compute'
	...
	current_cursor=fl_waiton;
		...
		if(strcmp(rseed_string,''))
		  eval([varname '=fBmWoodChan(N,H,tmax,1);']);
          
          chaine=[varname '=fBmWoodChan(',num2str(N),...
          ',[',num2str(H),'],',num2str(tmax),',1);'];
          
		else
	          rseed=str2num(rseed_string);
	          eval([varname '=fBmWoodChan(N,H,tmax,1,rseed);']);
              
          chaine=[varname '=fBmWoodChan(',num2str(N),...
          ',[',num2str(H),'],',num2str(tmax),',1,',num2str(rseed),');'];    
